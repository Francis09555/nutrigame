...
function updateHud(){
 ...
 const oldBuild=G.buildHudState||{},newBuild={},weaponSlots=Object.entries(player.weapons).map(([id,w])=>{const state=`${w.level}:${!!w.evolved}`;newBuild['w:'+id]=state;const changed=oldBuild['w:'+id]!==state;return `<div class="build-slot weapon-build-slot ${w.evolved?'evolved':''} ${changed?'build-changed':''}" title="${weaponDefs[id].name}${w.evolved?' → '+weaponDefs[id].evolved:''}"><span>${weaponDefs[id].icon}</span><b>Lv.${w.level}</b>${w.evolved?'<em>★</em>':''}</div>`}).join(''),passiveSlots=Object.entries(player.passives).filter(([,level])=>level>0).map(([id,level])=>{const state=String(level);newBuild['p:'+id]=state;const changed=oldBuild['p:'+id]!==state;return `<div class="build-slot passive-build-slot ${changed?'build-changed':''}" title="${passives[id]?.[0]||id}"><span>${passives[id]?.[1]||'◆'}</span><b>Lv.${level}</b></div>`}).join(''),trayHTML=`<div class="build-row weapon-build-row">${weaponSlots}</div>${passiveSlots?`<div class="build-row passive-build-row">${passiveSlots}</div>`:''}`;if(trayHTML!==G.lastTrayHTML){G.lastTrayHTML=trayHTML;G.buildHudState=newBuild;$('#weaponTray').innerHTML=trayHTML}
}
