...
function gainXp(v){player.xp+=v*player.xpGain;let gained=0;while(player.xp>=player.nextXp){player.xp-=player.nextXp;player.level++;gained++;
 ...
 if(gained){player.pendingUpgrades=(player.pendingUpgrades||0)+gained;if($('#choiceModal').classList.contains('hidden'))showLevelUp()}}
...
function finishQuizRound(){
 ...
 if(ok){G.score+=500;player.hp=Math.min(player.maxHp,player.hp+25);gainXp(player.nextXp*.35);$('#quizFeedback').textContent=`✅ Challenge passed: ${quizRound.correct}/3 — healing, XP, and +500 score`;if(typeof Starter!=='undefined')Starter.claim('quiz',`quiz:${G.runId}:${G.quizDone}`);sfx('level')}
 ...
 quizRound=null;setTimeout(()=>{$('#quizModal').classList.add('hidden');G.paused=false},1250);
}
