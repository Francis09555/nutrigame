...
function upgradePool(){
 let arr=[],weaponCount=Object.keys(player.weapons).length;
 for(const [id,d] of Object.entries(weaponDefs)){
  const w=player.weapons[id];
  if(!w){if(weaponCount<6)arr.push({type:'weapon',id,name:d.name,icon:d.icon,desc:'Unlock: '+d.desc});continue}
...
function finishQuizRound(){
 ...
 if(ok){G.score+=500;player.hp=Math.min(player.maxHp,player.hp+25);gainXp(player.nextXp*.35);$('#quizFeedback').textContent=`✅ Challenge passed: ${quizRound.correct}/3 — healing, XP, and +500 score`;sfx('level')}
...
function updateHud(){
 ...
 const oldBuild=G.buildHudState||{},newBuild={},trayHTML=Object.entries(player.weapons).slice(0,6).map(([id,w])=>{const state=`${w.level}:${!!w.evolved}`;newBuild[id]=state;const changed=oldBuild[id]!==state;return `<div class="compact-weapon-slot ${w.evolved?'evolved':''} ${changed?'build-changed':''}"><span>${weaponDefs[id].icon}</span><b>Lv.${w.level}</b>${w.evolved?'<em>★</em>':''}</div>`}).join('');if(trayHTML!==G.lastTrayHTML){G.lastTrayHTML=trayHTML;G.buildHudState=newBuild;$('#weaponTray').innerHTML=trayHTML}
}
