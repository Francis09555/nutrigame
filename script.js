...
function updateHud(){
 ...
 const trayHTML=Object.entries(player.weapons).map(([id,w])=>`<div class="weapon-slot ${w.evolved?'evolved':''}" data-weapon="${id}" title="${weaponDefs[id].name}"><span class="weapon-slot-icon">${weaponDefs[id].icon}</span><span class="weapon-slot-name">${w.evolved?weaponDefs[id].evolved:weaponDefs[id].name}</span><b>${w.evolved?'⭐ Evolution · Lv.6':'Lv.'+w.level}</b><span class="weapon-cooldown"><i></i></span></div>`).join('');if(trayHTML!==G.lastTrayHTML){G.lastTrayHTML=trayHTML;$('#weaponTray').innerHTML=trayHTML}for(const [id,w] of Object.entries(player.weapons)){const fill=$(`#weaponTray [data-weapon="${id}"] .weapon-cooldown i`);if(fill){const ready=!w.maxCd||w.cd<=0?1:clamp(1-w.cd/w.maxCd,0,1);fill.style.width=ready*100+'%'}}
}
