...
let W=innerWidth,H=innerHeight,DPR=1,resizeFrame=0;
function resize(){
 // Follow the actual visible browser area, including mobile address-bar and
 // orientation changes. DPR is capped to protect performance on 3x/4x phones.
 const viewport=window.visualViewport,Wnext=Math.max(320,Math.round(viewport?.width||innerWidth)),Hnext=Math.max(320,Math.round(viewport?.height||innerHeight));
 DPR=Math.min(devicePixelRatio||1,2);W=Wnext;H=Hnext;
 document.documentElement.style.setProperty('--app-width',W+'px');document.documentElement.style.setProperty('--app-height',H+'px');
...
function requestResize(){cancelAnimationFrame(resizeFrame);resizeFrame=requestAnimationFrame(resize)}
addEventListener('resize',requestResize);addEventListener('orientationchange',()=>setTimeout(requestResize,120));addEventListener('fullscreenchange',requestResize);window.visualViewport?.addEventListener('resize',requestResize);resize();
const defaults={unlocked:1,highScore:0,music:true,sfx:true,shake:true,volume:.55,companions:[],profile:null,coins:0,joystick:{x:null,y:null,size:120,opacity:.5},records:{classic:{highestLevel:0,highestScore:0,longestTime:0,totalKills:0,bosses:0},endless:{longestTime:0,highestScore:0,highestLevel:0,bosses:0,bestBosses:0}}};
const loadedSave=JSON.parse(localStorage.getItem('nutritionQuestSave')||'{}');
let save={...defaults,...loadedSave,joystick:{...defaults.joystick,...(loadedSave.joystick||{})},records:{classic:{...defaults.records.classic,...(loadedSave.records?.classic||{})},endless:{...defaults.records.endless,...(loadedSave.records?.endless||{})}}};
...
const weaponBaseCooldown={carrot:.65,apple:1.4,broccoli:2.4,banana:2.2,water:1.6,milk:2.5,fish:1.9,vitamin:2.8,rice:1.5,orange:1.35,tomato:2.15,corn:.9,cucumber:1.65,smoothie:1.45,chakram:1.7,laser:2.8,leaf:1.15,pulse:2.4,seed:.48,egg:0};
...
const quizzes=window.NQ_QUIZZES||[];
let quizDeck=[];
function shuffleQuizzes(){quizDeck=[...quizzes];for(let i=quizDeck.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[quizDeck[i],quizDeck[j]]=[quizDeck[j],quizDeck[i]]}}
...
const Online={
 ...
 async syncProfile(){if(!this.enabled()||!navigator.onLine||!save.profile)return{offline:true};try{return await this.call('sync-profile',{playerName:save.profile.name,avatar:save.profile.avatar})}catch(e){return{error:e.message||'Profile sync failed'}}},
...
// ---------- Menus ----------
function showScreen(id){if(id==='progressionAdmin'&&(typeof Starter==='undefined'||!Starter.data?.adminRole)){toast('Supabase administrator access required');id='menu'}$$('.screen').forEach(x=>x.classList.remove('active'));$('#'+id).classList.add('active');$('#hud').classList.add('hidden');$('#touchControls').classList.remove('game-on');G.running=false;updateSummary();if(id==='global')Online.load();if(id==='roulette'&&typeof Starter!=='undefined')Starter.refresh();if(id==='progressionAdmin'&&typeof Starter!=='undefined')Starter.adminList()}
...
$('#registerBtn').onclick=async()=>{const name=$('#registerName').value.trim();if(!name){$('#registerError').textContent='Please enter a player name.';return}const previous=save.profile,created={name,avatar:selectedAvatar,registrationDate:previous?.registrationDate||new Date().toISOString(),totalGames:0,totalWins:0,totalPlayTime:0,totalKills:0,favoriteWeapon:null,evolutionsUnlocked:[]};save.profile=created;store();$('#registerBtn').disabled=true;$('#registerError').textContent=navigator.onLine?'Checking player name…':'Saved offline — profile will sync later';const sync=await Online.syncProfile();$('#registerBtn').disabled=false;if(sync?.error){save.profile=previous||null;store();$('#registerError').textContent=sync.error.includes('already')?'That player name is already registered to another account. Choose a different name.':sync.error;return}$('#registerError').textContent='';$('#settingsName').value=name;audioStart();showScreen('menu');if(typeof Starter!=='undefined')Starter.refresh(true)};
$$('[data-action]').forEach(b=>b.onclick=()=>{audioStart();sfx('click');let a=b.dataset.action;if(a==='exit'){alert('Thanks for playing Nutrition Quest! You may now close this tab.');}else if(a==='multiplayer'&&typeof Starter!=='undefined')Starter.requestMultiplayer();else showScreen(a)});$$('[data-back]').forEach(b=>b.onclick=()=>{if(typeof joystickAdjusting!=='undefined'&&joystickAdjusting)beginJoystickAdjustment();showScreen('menu')});
$('#classicMode').onclick=()=>showScreen('levels');
$('#endlessMode').onclick=()=>Starter.requestRun(1,'endless');
...
function loadSettings(){$('#musicToggle').checked=save.music;$('#sfxToggle').checked=save.sfx;$('#shakeToggle').checked=save.shake;$('#volume').value=save.volume;$('#settingsName').value=save.profile?.name||'';$('#joystickSize').value=save.joystick.size;$('#joystickOpacity').value=save.joystick.opacity;setTimeout(applyJoystickSettings,0)}loadSettings();
$('#saveName').onclick=async()=>{const name=$('#settingsName').value.trim();if(!name)return toast('Player name cannot be empty');const oldName=save.profile?.name||'';save.profile={...(save.profile||{avatar:'🦸'}),name};store();const sync=await Online.syncProfile();if(sync?.error){save.profile.name=oldName;$('#settingsName').value=oldName;store();toast(sync.error.includes('already')?'That name belongs to another player':sync.error);return}updateSummary();toast(sync?.offline?'Name saved locally; it will sync when online':'Player name saved!')};
...
['music','sfx','shake'].forEach(k=>$('#'+k+'Toggle').onchange=e=>{save[k]=e.target.checked;store();startMusic()});$('#volume').oninput=e=>{save.volume=+e.target.value;store()};$('#joystickSize').oninput=e=>{save.joystick.size=+e.target.value;applyJoystickSettings();store()};$('#joystickOpacity').oninput=e=>{save.joystick.opacity=+e.target.value;applyJoystickSettings();store()};$('#adjustJoystick').onclick=()=>beginJoystickAdjustment();$('#resetJoystick').onclick=()=>{save.joystick={...defaults.joystick};$('#joystickSize').value=save.joystick.size;$('#joystickOpacity').value=save.joystick.opacity;applyJoystickSettings();store();toast('Joystick position reset')};$('#resetSave').onclick=()=>{if(confirm('Reset local progress and settings? Your online account identity will be kept so your registered name remains available to you.')){const auth=save.onlineAuth||null,oldProfile=save.profile,pending=save.pendingOnline||[];save={...defaults,joystick:{...defaults.joystick},records:{classic:{...defaults.records.classic},endless:{...defaults.records.endless}},onlineAuth:auth,pendingOnline:pending,profile:null};selectedAvatar=oldProfile?.avatar||'🦸';$('#registerName').value=oldProfile?.name||'';$('#registerError').textContent='';$$('.avatar-choice').forEach(b=>b.classList.toggle('selected',b.dataset.avatar===selectedAvatar));store();loadSettings();showScreen('register')}};
...
function evolvedWeaponCount(){return player?Object.values(player.weapons).filter(w=>w.evolved||w.level>=6).length:0}
function evolutionPressure(){
 // From 10–15 minutes the world adapts to strong builds. Pressure only begins
 // once at least three weapons have evolved and reaches a safe performance cap.
 if(!isSurvivalMode()||G.time<600)return{count:evolvedWeaponCount(),hp:1,damage:1,spawn:0,cap:0};
 const count=evolvedWeaponCount(),extra=Math.max(0,Math.min(5,count)-2),ramp=clamp((G.time-600)/300,0,1);
 return{count,hp:1+extra*.22*ramp,damage:1+extra*.11*ramp,spawn:extra*.48*ramp,cap:extra*45};
}
function evolvedArsenalDamageFactor(){
 // Damage-only diminishing returns keep 3–5 simultaneous evolutions powerful
 // without letting their combined output erase every late-game enemy instantly.
 const extra=Math.max(0,Math.min(5,evolvedWeaponCount())-2);
 return 1-extra*.06;
}
function spawnEnemy(type,x,y,small=false){let d=enemyDefs[type],minutes=G.time/60,pressure=evolutionPressure(),scale=(isSurvivalMode()?endlessCurve(minutes):(.78+(G.stage-1)*.19+G.time/G.duration*.65))*pressure.hp,elite=isSurvivalMode()&&!G.spawningSwarm&&minutes>3&&(G.eliteCount||0)<8&&Math.random()<Math.min(.04,(minutes-3)*.0025);if(elite){scale*=1.75;G.eliteCount=(G.eliteCount||0)+1;}if(x==null){let a=rnd(0,TAU),r=Math.max(W,H)*.7+100;x=player.x+Math.cos(a)*r;y=player.y+Math.sin(a)*r}enemies.push({type,x,y,r:d.size*(small?.65:1),hp:d.hp*scale*(small?.45:1),maxHp:d.hp*scale,damage:d.damage*(scale/pressure.hp)*pressure.damage,speed:d.speed*(isSurvivalMode()?Math.min(1.45,1.02+minutes*.055):1.5)*(1+(G.stage-1)*.025),icon:d.icon,cd:rnd(.2,2),hit:0,state:0,small,elite,dead:false});}
...
function updateWeapons(dt){
 for(const [id,w] of Object.entries(player.weapons)){
  ...
  w.cd=(weaponBaseCooldown[id]||1)/rate*(1-player.cooldown);w.maxCd=w.cd;
  ...
  if(id==='leaf'){
   // Leaf Storm attacks in readable gusts. A randomized 0.4–0.7 second gap
   // prevents the attack-speed stat from turning it into a continuous stream.
   const naturalCd=Math.max(.08,w.cd),delay=clamp(naturalCd*rnd(.9,1.1),.4,.7),originalCount=e?18:4+lv+player.count,count=e?12:3+Math.ceil(lv/2)+player.count;
   // Fewer visible leaves deal proportionally consolidated wave damage, and
   // enforced-delay compensation preserves the DPS that attack speed earned.
   const wavePower=(originalCount/count)*Math.max(1,delay/naturalCd),phase=rnd(0,TAU);w.cd=delay;w.maxCd=delay;
   for(let i=0;i<count;i++){const aa=phase+i/count*TAU;shoot(player.x,player.y,aa,e?360:240,(e?50:12+lv*5)*wavePower,'leaf',e?3.8:2.4,{r:e?13:8,spiral:e?.13:.06,pierce:e?4:1,statusSlow:e?2:1,trail:e,critBonus:.04,leafWave:true,visualLife:delay*1.65})}
   burst(player.x,player.y,e?'#a7ff73':'#7ee67a',e?16:8);
  }
  if(id==='pulse'){
...
function updateProjectiles(dt){
 for(const p of projectiles){
  p.age+=dt;p.life-=dt;if(p.leafWave&&p.age>p.visualLife)p.hiddenVisual=true;
  if(p.trail&&!p.hiddenVisual&&Math.random()<.7)particles.push({x:p.x,y:p.y,vx:rnd(-25,25),vy:rnd(-25,25),life:.28,max:1,color:'#ffe65d',r:rnd(2,5)});
  ...
  if(p.kind==='pulse'||p.kind==='starterPulse'){p.r=Math.min(p.maxR,p.r+p.growth*dt);p.x+=0;p.y+=0}
  ...
  const collisionCandidates=(p.kind==='beam'||p.kind==='laserBeam'||p.kind==='pulse'||p.kind==='starterPulse')?enemies:nearbyEnemies(p.x,p.y,p.r+45);
  for(const e of collisionCandidates)if(!e.dead&&!p.hit.has(e)){
   ...
   if(hit){
    p.hit.add(e);const wasCrit=hurtEnemy(e,p.dmg*((p.kind==='pulse'||p.kind==='starterPulse')?player.areaDamage:1),p.knock||8,Math.atan2(p.vy,p.vx),p.critBonus||0);if(p.statusSlow)e.slowTimer=Math.max(e.slowTimer||0,p.statusSlow*player.statusDuration);
...
function updateEnemies(dt){if((typeof DEV!=='undefined'&&DEV.freezeEnemies)||(G.adminFreezeUntil||0)>G.time)return;for(let e of enemies){if(e.dead)continue;e.cd-=dt;e.hit-=dt;let target=player;if(G.mode==='multiplayer'&&MP.isHost){for(const rp of MP.remotePlayers.values())if(rp.alive&&Math.hypot(rp.x-e.x,rp.y-e.y)<Math.hypot(target.x-e.x,target.y-e.y))target=rp}let dx=target.x-e.x,dy=target.y-e.y,d=Math.hypot(dx,dy)||1,a=Math.atan2(dy,dx);e.targetId=target===player?MP.uid?.()||'local':target.id;if(e===G.boss){updateBoss(e,dt,a,d);continue}e.slowTimer=Math.max(0,(e.slowTimer||0)-dt);let idlePressure=isSurvivalMode()&&G.time>300&&player.idle>4?1+Math.min(.75,(player.idle-4)*.045):1,sp=e.speed*(e.slowTimer>0?.48:1)*idlePressure*(typeof DEV!=='undefined'?DEV.enemySpeed:1);if(e.type==='Soda'&&d<430){sp=d<240?-sp*.5:sp*.2;if(e.cd<=0){enemyProjectile(e,a,190);e.cd=2.2}}
 ...
 e.x+=dx/d*sp*dt;e.y+=dy/d*sp*dt;if(d<e.r+player.r){if(target===player)hurtPlayer(e.damage);else if(G.mode==='multiplayer'&&MP.isHost&&!target.adminGod&&!target.shield&&(target.shieldUntil||0)<=G.time){target.hp=Math.max(0,target.hp-e.damage);MP.queueDamage(target.id,e.damage)}if(e.type==='IceCream'&&target===player)player.slow=2;e.x-=dx/d*18;e.y-=dy/d*18}}
...
function updateEnemyShots(dt){
 ...
 for(const p of enemyShots){
  ...
  else if(G.mode==='multiplayer'&&MP.isHost&&p.age>(p.grace||.2)){for(const rp of MP.remotePlayers.values())if(rp.alive&&!rp.adminGod&&!rp.shield&&(rp.shieldUntil||0)<=G.time&&Math.hypot(p.x-rp.x,p.y-rp.y)<p.r*.72+15){rp.hp=Math.max(0,rp.hp-p.dmg);MP.queueDamage(rp.id,p.dmg);p.life=0;break}}
 }
 enemyShots=enemyShots.filter(p=>p.life>0);
}

// ---------- Progression, upgrades, quiz ----------
function gainXp(v){player.xp+=v*player.xpGain;let gained=0;while(player.xp>=player.nextXp){player.xp-=player.nextXp;player.level++;gained++;
 ...
 if(gained){player.pendingUpgrades=(player.pendingUpgrades||0)+gained;if(!G.suppressLevelUp&&$('#choiceModal').classList.contains('hidden'))showLevelUp()}}
function upgradePool(){
 let arr=[];
 for(const [id,d] of Object.entries(weaponDefs)){
  const w=player.weapons[id];
  if(!w){arr.push({type:'weapon',id,name:d.name,icon:d.icon,desc:'Unlock: '+d.desc});continue}
  if(w.level<5)arr.push({type:'weapon',id,targetLevel:w.level+1,name:d.name+' Lv.'+(w.level+1),icon:d.icon,desc:'Improves its unique damage, physics, range, and attack pattern'});
  else if(w.level===5&&!w.evolved)arr.push({type:'weapon',id,targetLevel:6,evolution:true,name:d.name+' Lv.6',icon:d.icon,desc:'Evolves into '+d.evolved+' — a completely new maximum form'});
 }
 for(const [id,d] of Object.entries(passives))if((player.passives[id]||0)<5)arr.push({type:'passive',id,name:d[0],icon:d[1],desc:'Improve '+d[0].toLowerCase()});
 for(const [id,d] of Object.entries(ultimateDefs))if(!player.ultimates[id])arr.push({type:'ultimate',id,name:d[0],icon:d[1],desc:d[2],rare:true});
 const companions=[['apple','Baby Apple','🍎'],['bee','Healthy Bee','🐝'],['fairy','Vitamin Fairy','🧚'],['farmer','Mini Farmer','🧑‍🌾'],['chicken','Golden Chicken','🐔']];
 if(save.unlocked>=3){if(!player.companion)for(const c of companions)arr.push({type:'companion',id:c[0],name:c[1],icon:c[2],desc:'A unique following companion'});else if(player.companion.level<5)arr.push({type:'companionLevel',id:player.companion.type,name:player.companion.name+' Lv.'+(player.companion.level+1),icon:player.companion.icon,desc:'Improve companion AI, attack, support, and projectiles'})}
 return arr;
}
function showLevelUp(){if((player.pendingUpgrades||0)>0)player.pendingUpgrades--;if(G.mode!=='multiplayer')G.paused=true;sfx('level');let pool=upgradePool(),choices=[];if(G.forceQuality){const quality=pool.filter(x=>x.type==='weapon'||x.type==='ultimate'||x.evolution);if(quality.length>=3)pool=quality;G.forceQuality=false;}while(choices.length<3&&pool.length){let weights=pool.filter(x=>!x.rare||Math.random()<.2+player.luck*.05);let q=pick(weights.length?weights:pool);choices.push(q);pool=pool.filter(x=>x!==q)}$('#choices').innerHTML='';for(let c of choices){let b=document.createElement('button');b.className='choice'+(c.rare?' rare':'')+(c.evolution?' evolution-choice':'');b.innerHTML=`<span class="icon">${c.icon}</span><b>${c.name}</b><small>${c.desc}</small>`;b.onclick=()=>applyUpgrade(c);$('#choices').appendChild(b)}$('#choiceModal').classList.remove('hidden')}
function applyUpgrade(c){if(c.type==='weapon'){if(!player.weapons[c.id])player.weapons[c.id]={level:1,cd:0,evolved:false};else if(player.weapons[c.id].level===5){player.weapons[c.id].level=6;player.weapons[c.id].evolved=true;G.evolution=1.15;sfx('evolve');burst(player.x,player.y,'#ffe45c',90);burst(player.x,player.y,'#ffffff',45);}else player.weapons[c.id].level=Math.min(5,player.weapons[c.id].level+1)}if(c.type==='passive'){player.passives[c.id]=(player.passives[c.id]||0)+1;let n=player.passives[c.id];if(c.id==='speed')player.speed*=1.08;if(c.id==='damage')player.damage*=1.12;if(c.id==='attackSpeed')player.attackSpeed*=1.09;if(c.id==='cooldown')player.cooldown=Math.min(.45,player.cooldown+.06);if(c.id==='crit')player.crit+=.04;if(c.id==='critDamage')player.critDamage+=.18;if(c.id==='health'){player.maxHp+=20;player.hp+=20}if(c.id==='regen')player.regen+=.35;if(c.id==='armor')player.armor++;if(c.id==='projectileSpeed')player.projectileSpeed+=.12;if(c.id==='size')player.size+=.12;if(c.id==='count')player.count++;if(c.id==='knockback')player.knockback+=.18;if(c.id==='explosionRadius')player.explosionRadius+=.14;if(c.id==='areaDamage')player.areaDamage+=.12;if(c.id==='piercing')player.piercing++;if(c.id==='ricochet')player.ricochet++;if(c.id==='bounce')player.bounce++;if(c.id==='bossDamage')player.bossDamage+=.15;if(c.id==='status')player.statusDuration+=.18;if(c.id==='pickup')player.pickup+=25;if(c.id==='xp')player.xpGain+=.1;if(c.id==='luck')player.luck++}if(c.type==='ultimate')player.ultimates[c.id]={cd:3,max:18};if(c.type==='companion')player.companion={type:c.id,name:c.name,icon:c.icon,level:1,x:player.x-50,y:player.y+35,cd:0,phase:0};if(c.type==='companionLevel')player.companion.level++;$('#choiceModal').classList.add('hidden');toast(`${c.icon} ${c.name}`);if((player.pendingUpgrades||0)>0){if(G.mode!=='multiplayer')G.paused=true;setTimeout(()=>{if(G.running)showLevelUp()},80)}else G.paused=false}
...
let quizTimerRAF=0,quizDeadline=0,quizRound=null,currentQuiz=null,quizAnswered=false;
function showQuiz(){
 if(!quizDeck.length)shuffleQuizzes();G.paused=true;quizRound={remaining:3,correct:0};$('#quizModal').classList.remove('hidden');nextQuizQuestion();
}
function nextQuizQuestion(){
 cancelAnimationFrame(quizTimerRAF);if(!quizRound||quizRound.remaining<=0)return finishQuizRound();
 if(!quizDeck.length)shuffleQuizzes();currentQuiz=quizDeck.shift();quizAnswered=false;quizRound.remaining--;
 $('#quizQuestion').textContent=currentQuiz[0];$('#quizAnswers').innerHTML='';$('#quizFeedback').textContent=`Question ${3-quizRound.remaining} of 3`;
 currentQuiz[1].forEach((answer,i)=>{const b=document.createElement('button');b.textContent=answer;b.onclick=()=>selectQuizAnswer(i);$('#quizAnswers').appendChild(b)});
 quizDeadline=performance.now()+5000;updateQuizTimer();
}
function updateQuizTimer(){
 if(quizAnswered)return;const left=Math.max(0,(quizDeadline-performance.now())/1000),pct=left/5*100;
 $('#quizTimerText').textContent=left.toFixed(1);$('#quizTimerBar').style.width=pct+'%';$('#quizTimerBar').classList.toggle('danger',left<=2);
 if(left<=0)return selectQuizAnswer(-1);quizTimerRAF=requestAnimationFrame(updateQuizTimer);
}
function selectQuizAnswer(index){
 if(quizAnswered)return;quizAnswered=true;cancelAnimationFrame(quizTimerRAF);const correct=index===currentQuiz[2],buttons=[...$('#quizAnswers').children];buttons.forEach((b,i)=>{b.disabled=true;if(i===currentQuiz[2])b.classList.add('correct');else if(i===index)b.classList.add('wrong')});
 if(correct){quizRound.correct++;$('#quizFeedback').textContent='✅ Correct!'}else $('#quizFeedback').textContent=`${index<0?'⏱ Time expired!':'❌ Incorrect.'} Correct answer: ${currentQuiz[1][currentQuiz[2]]}`;
 setTimeout(nextQuizQuestion,950);
}
function finishQuizRound(){
 cancelAnimationFrame(quizTimerRAF);const ok=quizRound.correct>=2;quizAnswered=true;
 if(ok){G.score+=500;player.hp=Math.min(player.maxHp,player.hp+25);G.suppressLevelUp=true;gainXp(player.nextXp*.35);G.suppressLevelUp=false;$('#quizFeedback').textContent=`✅ Challenge passed: ${quizRound.correct}/3 — healing, XP, and +500 score`;if(typeof Starter!=='undefined')Starter.claim('quiz',`quiz:${G.runId}:${G.quizDone}`);sfx('level')}
 else{G.score=Math.max(0,G.score-200);player.hp=Math.max(1,player.hp-15);$('#quizFeedback').textContent=`❌ Challenge result: ${quizRound.correct}/3 — review and try again!`;sfx('bad')}
 quizRound=null;setTimeout(()=>{$('#quizModal').classList.add('hidden');if((player.pendingUpgrades||0)>0)showLevelUp();else G.paused=false},1250);
}

// ---------- Movement, collision, companions ----------
function dash(){if(!G.running||G.paused||(player.dashCd>0&&!(typeof DEV!=='undefined'&&DEV.infiniteDash)))return;let dx=(keys.d||keys.arrowright?1:0)-(keys.a||keys.arrowleft?1:0),dy=(keys.s||keys.arrowdown?1:0)-(keys.w||keys.arrowup?1:0);if(!dx&&!dy){dx=player.dir.x;dy=player.dir.y}let d=Math.hypot(dx,dy)||1;player.dir={x:dx/d,y:dy/d};player.dashTime=.2;player.dashCd=player.dashMax;player.invuln=.35;sfx('dash');burst(player.x,player.y,'#a7ffff',20)}
function updatePlayer(dt){if(player.dead){player.invuln=Math.max(0,player.invuln-dt);return}let dx=(keys.d||keys.arrowright?1:0)-(keys.a||keys.arrowleft?1:0),dy=(keys.s||keys.arrowdown?1:0)-(keys.w||keys.arrowup?1:0);if(touch.active){dx=touch.x;dy=touch.y}let d=Math.hypot(dx,dy);if(d){dx/=d;dy/=d;player.dir={x:dx,y:dy};player.idle=0}else player.idle=(player.idle||0)+dt;let sp=player.speed*(player.slow>0?.62:1)*(player.dashTime>0?4:1);player.x+=dx*sp*dt;player.y+=dy*sp*dt;if(player.dashTime>0){for(let i=0;i<3;i++)particles.push({x:player.x-rnd(-10,10),y:player.y-rnd(-10,10),vx:-player.dir.x*rnd(60,180)+rnd(-30,30),vy:-player.dir.y*rnd(60,180)+rnd(-30,30),life:.28,max:1,color:'#a7ffff',r:rnd(3,7)})}const limit=worldLimit();player.x=clamp(player.x,-limit,limit);player.y=clamp(player.y,-limit,limit);player.dashCd=Math.max(0,player.dashCd-dt);player.dashTime=Math.max(0,player.dashTime-dt);player.invuln=Math.max(0,player.invuln-dt);player.slow=Math.max(0,player.slow-dt);player.hp=Math.min(player.maxHp,player.hp+player.regen*dt);for(let g of gems){let dd=Math.hypot(g.x-player.x,g.y-player.y);if(dd<player.pickup){g.x+=(player.x-g.x)*dt*clamp(500/dd,5,18);g.y+=(player.y-g.y)*dt*clamp(500/dd,5,18)}if(dd<player.r+9){g.dead=true;if(G.mode==='multiplayer'&&!MP.isHost&&g.netId)MP.broadcast('collect_gem',{id:MP.uid(),gemId:g.netId});gainXp(g.v);if(typeof Starter!=='undefined')Starter.gainXp(g.v);sfx('xp')}}gems=gems.filter(g=>!g.dead);for(let t of traps){t.life-=dt;t.cd-=dt;if(t.cd<=0){for(let e of enemies)if(Math.hypot(e.x-t.x,e.y-t.y)<t.r+e.r){hurtEnemy(e,t.dmg,12);e.slowTimer=Math.max(e.slowTimer||0,t.slow||.5);e.x+=(e.x-t.x)*.04;e.y+=(e.y-t.y)*.04}t.cd=.45}}traps=traps.filter(t=>t.life>0);updateCompanion(dt)}
...
function drawProjectile(p){
 ...
 if(p.kind==='pulse'||p.kind==='starterPulse'){ctx.save();ctx.globalAlpha=clamp(p.life*1.4,0,.75);ctx.strokeStyle='#fff477';ctx.shadowColor='#7effa0';ctx.shadowBlur=18;ctx.lineWidth=10;ctx.beginPath();ctx.arc(q.x,q.y,p.r,0,TAU);ctx.stroke();ctx.restore();return}
 const icons={carrot:'🥕',apple:'🍎',broccoli:'🥦',water:'💧',fish:'🐟',vitamin:'💊',rice:'•',orange:'🍊',tomato:'🍅',corn:'🌽',cucumber:'🥒',smoothie:'🫧',chakram:'🥝',leaf:'🍃',seed:'🌱',pulse:'✨',eggShot:'🥚',starterChili:'🌶️',starterCabbage:'🥬',starterOnion:'🧅',starterPotato:'🥔',starterEggplant:'🍆',starterCoconut:'🥥',starterPineapple:'🍍',starterPeanut:'🥜',starterPepper:'🫑'};
 ctx.save();
...
function stepSimulation(dt,background=false){
 ...
 if(!mpClient){if(!player.dead){updateWeapons(dt);if(typeof Starter!=='undefined')Starter.update(dt);updateUltimates(dt)}rebuildSpatialGrid();updateProjectiles(dt);updateEnemies(dt);updateEnemyShots(dt)}else updateParticles(dt);
...
function updateHud(){
 ...
 const oldBuild=G.buildHudState||{},newBuild={},allWeaponEntries=Object.entries(player.weapons);G.weaponHudOrder=G.weaponHudOrder||[];for(const [id,w] of allWeaponEntries){const state=`${w.level}:${!!w.evolved}`;newBuild[id]=state;if(!G.weaponHudOrder.includes(id))G.weaponHudOrder.push(id);else if(oldBuild[id]!==undefined&&oldBuild[id]!==state){G.weaponHudOrder=G.weaponHudOrder.filter(x=>x!==id);G.weaponHudOrder.push(id)}}G.weaponHudOrder=G.weaponHudOrder.filter(id=>player.weapons[id]);const visibleIds=G.weaponHudOrder,weaponCount=visibleIds.length,rows=weaponCount>10?2:1,columns=Math.max(1,Math.ceil(weaponCount/rows)),baseSize=weaponCount<=6?48:weaponCount<=12?40:weaponCount<=18?32:28,available=Math.max(240,W-90),slotSize=Math.max(22,Math.min(baseSize,Math.floor((Math.min(720,available)-(columns-1)*5)/columns))),trayHTML=visibleIds.map(id=>{const w=player.weapons[id],changed=oldBuild[id]!==newBuild[id];return `<div class="compact-weapon-slot ${w.evolved?'evolved':''} ${changed?'build-changed':''}"><span>${weaponDefs[id].icon}</span><b>Lv.${w.level}</b>${w.evolved?'<em>★</em>':''}</div>`}).join('');if(trayHTML!==G.lastTrayHTML){G.lastTrayHTML=trayHTML;G.buildHudState=newBuild;$('#weaponTray').innerHTML=trayHTML}$('#weaponTray').style.setProperty('--weapon-slot-size',slotSize+'px');$('#weaponTray').style.setProperty('--weapon-columns',columns);$('#weaponTray').style.setProperty('--weapon-hud-width',(columns*slotSize+(columns-1)*5)+'px')
}
...
function finishMultiplayerMatch(result={}){if(G.multiplayerResultsShown)return;G.multiplayerResultsShown=true;G.multiplayerGameOver=true;G.paused=true;projectiles=[];enemyShots=[];if(typeof MP!=='undefined')MP.active=false;if(result.score!=null)G.score=result.score;if(result.time!=null)G.time=result.time;showResult(false)}
function victory(){
 ...
 updateRecords();if(typeof Starter!=='undefined')Starter.claimRun();sfx('win');setTimeout(()=>showResult(true),500);
}
function endGame(win){
 G.paused=true;if(G.mode==='endless'){const earned=Math.floor(G.score/700);G.coins+=earned;save.coins=(save.coins||0)+earned}updateRecords();if(typeof Starter!=='undefined')Starter.claimRun();showResult(false);
...
function showResult(win){
 ...
 $('#resultTitle').textContent=win?(G.stage===10?'The City Is Healthy Again!':'Stage Cleared!'):(G.mode==='multiplayer'?'All Teammates Are Down':G.mode==='endless'?'Endless Run Complete':'The Junk Food Wave Won');
 ...
 $('#resultBody').innerHTML=win?(G.stage<10?`Level ${G.stage+1} is now unlocked.`:'The Junk Food King is defeated. Healthy foods return across the kingdom!'):(G.mode==='multiplayer'?'The co-op run ended because every connected hero was downed.':'Grow stronger, choose new upgrades, and try again.');
 $('#resultStats').innerHTML=`Player: <b>${save.profile?.avatar||'🦸'} ${save.profile?.name||'Healthy Hero'}</b><br>Mode: <b>${G.mode==='multiplayer'?'Online Co-op':G.mode==='endless'?'Endless Mode':'Classic Mode'}</b><br>Survival Time: <b>${formatClock(G.time)}</b> · Highest Level: <b>${player.level}</b><br>Score: <b>${Math.floor(G.score).toLocaleString()}</b> · Kills: <b>${player.kills}</b><br>Bosses Defeated: <b>${G.bossesDefeated}</b> · Coins Earned: <b>${G.coins}</b><br>${records}`;
 $('#continueBtn').textContent=G.mode==='multiplayer'?'Leave Room':(win&&G.mode==='classic'&&G.stage<10?'Next Level':'Play Again');
 $('#continueBtn').onclick=()=>{$('#resultModal').classList.add('hidden');if(G.mode==='multiplayer')MP.leave();else Starter.requestRun(win&&G.mode==='classic'&&G.stage<10?G.stage+1:G.stage,G.mode)};
}
$('#resultMenuBtn').onclick=async()=>{$('#resultModal').classList.add('hidden');G.running=false;if(G.mode==='multiplayer'){await MP.leave();return}showScreen('credits');startMusic();clearTimeout(G.creditTimer);G.creditTimer=setTimeout(()=>{if($('#credits').classList.contains('active'))showScreen('menu')},4500)};
$('#resultScoresBtn').onclick=async()=>{$('#resultModal').classList.add('hidden');G.running=false;if(G.mode==='multiplayer')await MP.leave();showScreen('scores')};
...
// ---------- Touch joystick ----------
const touch={active:false,x:0,y:0};let joy=$('#joystick'),knob=joy.querySelector('i'),joystickAdjusting=false,joystickDrag=false;
function applyJoystickSettings(){if(!joy||!save.joystick)return;const size=save.joystick.size||120;joy.style.width=joy.style.height=size+'px';joy.style.opacity=save.joystick.opacity??.5;knob.style.width=knob.style.height=Math.round(size*.4)+'px';knob.style.left=knob.style.top=Math.round(size*.3)+'px';if(save.joystick.x==null){joy.style.left='25px';joy.style.bottom='70px';joy.style.top='auto'}else{joy.style.left=Math.round(save.joystick.x*Math.max(0,W-size))+'px';joy.style.top=Math.round(save.joystick.y*Math.max(0,H-size))+'px';joy.style.bottom='auto'}}
function beginJoystickAdjustment(){joystickAdjusting=!joystickAdjusting;$('#touchControls').classList.toggle('adjusting',joystickAdjusting);$('#adjustJoystick').textContent=joystickAdjusting?'Finish Joystick Position':'Drag Joystick Position';if(joystickAdjusting){applyJoystickSettings();toast('Drag the joystick anywhere, then press Finish')}else{store();toast('Joystick position saved')}}
function joyMove(e){if(joystickAdjusting)return;let r=joy.getBoundingClientRect(),t=e.touches?e.touches[0]:e,dx=t.clientX-(r.left+r.width/2),dy=t.clientY-(r.top+r.height/2),d=Math.hypot(dx,dy),max=r.width/3;if(d>max){dx=dx/d*max;dy=dy/d*max}touch.x=dx/max;touch.y=dy/max;touch.active=true;knob.style.transform=`translate(${dx}px,${dy}px)`}
joy.addEventListener('pointerdown',e=>{if(!joystickAdjusting)return;joystickDrag=true;joy.setPointerCapture(e.pointerId);e.preventDefault()});joy.addEventListener('pointermove',e=>{if(!joystickAdjusting||!joystickDrag)return;const size=save.joystick.size,left=clamp(e.clientX-size/2,0,W-size),top=clamp(e.clientY-size/2,0,H-size);save.joystick.x=left/Math.max(1,W-size);save.joystick.y=top/Math.max(1,H-size);applyJoystickSettings();e.preventDefault()});joy.addEventListener('pointerup',()=>{if(joystickDrag){joystickDrag=false;store()}});
joy.addEventListener('touchstart',joyMove,{passive:false});joy.addEventListener('touchmove',e=>{if(!joystickAdjusting)e.preventDefault();joyMove(e)},{passive:false});joy.addEventListener('touchend',()=>{if(joystickAdjusting)return;touch.active=false;touch.x=touch.y=0;knob.style.transform='' });$('#dashTouch').ontouchstart=e=>{e.preventDefault();dash()};
addEventListener('resize',applyJoystickSettings);applyJoystickSettings();
