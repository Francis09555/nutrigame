...
const worldLimit=()=>G?.mode==='endless'?7000:1650;
...
const defaults={unlocked:1,highScore:0,music:true,sfx:true,shake:true,volume:.55,companions:[],profile:null,coins:0,records:{classic:{highestLevel:0,highestScore:0,longestTime:0,totalKills:0,bosses:0},endless:{longestTime:0,highestScore:0,highestLevel:0,bosses:0}}};
...
const Online={
 ...
 render(rows){const uid=save.onlineAuth?.user?.id,podium=$('#leaderPodium'),list=$('#leaderList');podium.innerHTML=rows.filter(r=>r.global_rank<=3).map(r=>`<article class="podium-card ${r.player_id===uid?'current':''}"><b>#${r.global_rank}</b><span class="avatar">${r.avatar}</span><b>${escapeHTML(r.player_name)}</b><span>${Number(r.score).toLocaleString()} pts</span><small>${formatClock(r.survival_time)}</small></article>`).join('');list.innerHTML='<div class="leader-row header"><span>Rank</span><span>Player</span><span>Score</span><span>Time</span><span>Level</span><span>Bosses</span><span>Date</span></div>'+rows.filter(r=>r.global_rank>3).map(r=>`<div class="leader-row ${r.player_id===uid?'current':''}"><b>#${r.global_rank}</b><span class="leader-player"><i>${r.avatar}</i>${escapeHTML(r.player_name)}</span><b>${Number(r.score).toLocaleString()}</b><span>${formatClock(r.survival_time)}</span><span>Lv.${r.level}</span><span>${r.bosses}</span><small>${new Date(r.achieved_at).toLocaleDateString()}</small></div>`).join('');const me=rows.find(r=>r.player_id===uid);$('#myGlobalRank').textContent=me?`Your Global Rank: #${me.global_rank}`:''}
};
...
function startGame(stage,mode='classic'){audioStart();G.running=true;G.paused=false;G.mode=mode;G.stage=stage;G.time=0;G.duration=mode==='endless'?Infinity:300+(stage-1)*30;G.score=0;G.boss=null;G.bossesDefeated=0;G.nextBossAt=mode==='endless'?600:300;G.coins=0;G.newRecords=[];G.weaponUse={};G.eliteCount=0;G.swarmCycle=0;G.swarm=null;G.nextSwarmAt=mode==='endless'?rnd(60,480):Infinity;G.shake=0;if(mode==='endless')Online.startRun();G.quizAt=mode==='endless'?[150,330]:[G.duration*.38,G.duration*.72];G.quizDone=0;player=makePlayer();enemies=[];projectiles=[];enemyShots=[];gems=[];particles=[];texts=[];traps=[];makeDecor();$$('.screen,.overlay').forEach(x=>x.classList.remove('active'));$$('.overlay').forEach(x=>x.classList.add('hidden'));$('#hud').classList.remove('hidden');$('#touchControls').classList.add('game-on');$('#bossHud').classList.add('hidden');startMusic();last=performance.now();requestAnimationFrame(loop);toast(`${maps[stage-1][3]} ${maps[stage-1][0]}`)}
function makeDecor(){decor=[];let seed=G.stage*991+(G.mode==='endless'?777:0),limit=worldLimit(),count=G.mode==='endless'?520:180;const rand=()=>((seed=Math.imul(seed,1664525)+1013904223|0)>>>0)/4294967296;for(let i=0;i<count;i++)decor.push({x:(rand()*2-1)*limit,y:(rand()*2-1)*limit,t:rand()>.35?maps[G.stage-1][3]:maps[G.stage-1][4],s:15+rand()*20})}
...
function spawnEnemy(type,x,y,small=false){let d=enemyDefs[type],minutes=G.time/60,scale=G.mode==='endless'?endlessCurve(minutes):(.78+(G.stage-1)*.19+G.time/G.duration*.65),elite=G.mode==='endless'&&!G.spawningSwarm&&minutes>3&&(G.eliteCount||0)<8&&Math.random()<Math.min(.04,(minutes-3)*.0025);if(elite){scale*=1.75;G.eliteCount=(G.eliteCount||0)+1;}if(x==null){let a=rnd(0,TAU),r=Math.max(W,H)*.7+100;x=player.x+Math.cos(a)*r;y=player.y+Math.sin(a)*r}enemies.push({type,x,y,r:d.size*(small?.65:1),hp:d.hp*scale*(small?.45:1),maxHp:d.hp*scale,damage:d.damage*scale,speed:d.speed*(G.mode==='endless'?Math.min(1.45,1.02+minutes*.055):1.5)*(1+(G.stage-1)*.025),icon:d.icon,cd:rnd(.2,2),hit:0,state:0,small,elite,dead:false});}
function availableEnemies(){const unlocked=G.mode==='endless'?2+Math.floor(G.time/120):2+G.stage+Math.floor(G.time/G.duration*3);return Object.keys(enemyDefs).slice(0,Math.min(10,unlocked))}
...
function spawnBoss(){let names=['Burger Baron','Fry Captain','Pizza Brute','Soda Sorcerer','Candy Crusher','Donut Phantom','Ice Cream Titan','Hotdog General','Chocolate Emperor','Junk Food King'],wave=G.mode==='endless'?Math.floor(G.time/600):G.stage,bossTier=G.mode==='endless'?Math.max(1,wave):G.stage,nameIndex=(bossTier-1)%names.length;let hp=G.mode==='endless'?1800*Math.pow(1.48,bossTier-1):1600*Math.pow(1.43,G.stage-1);G.boss={type:'Boss',name:names[nameIndex]+(G.mode==='endless'?` · Wave ${bossTier}`:''),icon:nameIndex===9?'👑':'👹',x:player.x+500,y:player.y,r:48+Math.min(20,bossTier*2),hp,maxHp:hp,damage:18+bossTier*5,speed:48+Math.min(35,bossTier*3),tier:bossTier,cd:1,ability:0,phase:1,hit:0,dead:false};G.bossEntrance=1.4;G.shake=14;enemies.push(G.boss);$('#bossHud').classList.remove('hidden');$('#bossName').textContent=G.boss.name;sfx('boss');startMusic();toast(`⚠ ${G.boss.name} has appeared!`)}
...
// ---------- Enemy and boss AI ----------
function enemyProjectile(e,a,speed=180,n=1,bounce=0){if(e===G.boss)tone(95,.14,'sawtooth',.07,120);else if(Math.random()<.12)tone(185,.05,'square',.025,-40);for(let i=0;i<n;i++){let aa=a+(i-(n-1)/2)*.22;enemyShots.push({x:e.x,y:e.y,vx:Math.cos(aa)*speed,vy:Math.sin(aa)*speed,r:e===G.boss?9:7,dmg:e.damage*.7,life:6,bounce})}}
function updateEnemies(dt){for(let e of enemies){if(e.dead)continue;e.cd-=dt;e.hit-=dt;let dx=player.x-e.x,dy=player.y-e.y,d=Math.hypot(dx,dy)||1,a=Math.atan2(dy,dx);if(e===G.boss){updateBoss(e,dt,a,d);continue}e.slowTimer=Math.max(0,(e.slowTimer||0)-dt);let idlePressure=G.mode==='endless'&&G.time>300&&player.idle>4?1+Math.min(.75,(player.idle-4)*.045):1,sp=e.speed*(e.slowTimer>0?.48:1)*idlePressure;if(e.type==='Soda'&&d<430){sp=d<240?-sp*.5:sp*.2;if(e.cd<=0){enemyProjectile(e,a,190);e.cd=2.2}}
 ...
 e.x+=dx/d*sp*dt;e.y+=dy/d*sp*dt;if(d<e.r+player.r){hurtPlayer(e.damage);if(e.type==='IceCream')player.slow=2;e.x-=dx/d*18;e.y-=dy/d*18}}
...
function updateBoss(b,dt,a,d){let tier=b.tier||G.stage;let phase=Math.min(tier>=10?5:3,1+Math.floor((1-b.hp/b.maxHp)*(tier>=10?5:3)));b.phase=phase;let rage=tier>=10&&b.hp/b.maxHp<.2?1.65:1;b.x+=Math.cos(a)*b.speed*rage*dt*(d>180?1:-.25);b.y+=Math.sin(a)*b.speed*rage*dt*(d>180?1:-.25);if(d<b.r+player.r)hurtPlayer(b.damage);if(b.cd>0)return;b.ability=(b.ability+1)%Math.min(10,2+tier);let ab=b.ability;b.cd=Math.max(.75,2.6-tier*.08)/rage;
 ...
 else if(ab===2){for(let i=0;i<10+tier;i++){let aa=rnd(0,TAU);enemyShots.push({x:player.x+Math.cos(aa)*rnd(180,450),y:player.y-500-rnd(0,300),vx:0,vy:260,r:10,dmg:b.damage*.55,life:5,bounce:0})}}
 ...
 else if(ab===6){enemyShots.push({x:b.x,y:b.y,vx:Math.cos(a)*340,vy:Math.sin(a)*340,r:18,dmg:b.damage,life:2,bounce:1})}
 ...
 else {for(let i=0;i<8;i++)enemyShots.push({x:player.x+rnd(-400,400),y:player.y-600-rnd(0,400),vx:0,vy:340,r:16,dmg:b.damage*.8,life:5,bounce:0})}}
function updateEnemyShots(dt){for(let p of enemyShots){p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;if(p.bounce&&Math.abs(p.x-player.x)>W*.6){p.vx*=-1;p.bounce--}if(Math.hypot(p.x-player.x,p.y-player.y)<p.r+player.r){hurtPlayer(p.dmg);p.life=0}}enemyShots=enemyShots.filter(p=>p.life>0)}
...
function gainXp(v){player.xp+=v*player.xpGain;while(player.xp>=player.nextXp){player.xp-=player.nextXp;player.level++;
 ...
 if(player.level===2)player.nextXp=3;else if(player.level===3)player.nextXp=6;else if(player.level===4)player.nextXp=10;else if(player.level===5)player.nextXp=15;else if(G.mode==='endless')player.nextXp=Math.floor(player.nextXp*(player.level<12?1.16:1.105)+4);else player.nextXp=Math.floor(player.nextXp*1.19+5);
 showLevelUp();break}}
...
function showLevelUp(){G.paused=true;sfx('level');let pool=upgradePool(),choices=[];if(G.forceQuality){const quality=pool.filter(x=>x.type==='weapon'||x.type==='ultimate'||x.evolution);if(quality.length>=3)pool=quality;G.forceQuality=false;}while(choices.length<3&&pool.length){let weights=pool.filter(x=>!x.rare||Math.random()<.2+player.luck*.05);let q=pick(weights.length?weights:pool);choices.push(q);pool=pool.filter(x=>x!==q)}$('#choices').innerHTML='';for(let c of choices){let b=document.createElement('button');b.className='choice'+(c.rare?' rare':'')+(c.evolution?' evolution-choice':'');b.innerHTML=`<span class="icon">${c.icon}</span><b>${c.name}</b><small>${c.desc}</small>`;b.onclick=()=>applyUpgrade(c);$('#choices').appendChild(b)}$('#choiceModal').classList.remove('hidden')}
...
function draw(){let m=maps[G.stage-1],sx=G.shake?rnd(-G.shake,G.shake):0,sy=G.shake?rnd(-G.shake,G.shake):0;G.shake*=.86;ctx.save();ctx.translate(sx,sy);if(G.bossEntrance>0){const z=1+Math.sin((1.4-G.bossEntrance)/1.4*Math.PI)*.07;ctx.translate(W/2,H/2);ctx.scale(z,z);ctx.translate(-W/2,-H/2)}ctx.fillStyle=m[1];ctx.fillRect(-10,-10,W+20,H+20);let grid=90,ox=(-G.cam.x+W/2)%grid,oy=(-G.cam.y+H/2)%grid;ctx.strokeStyle=m[2]+'25';ctx.lineWidth=2;for(let x=ox;x<W;x+=grid){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,H);ctx.stroke()}for(let y=oy;y<H;y+=grid){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke()}// Lightweight procedural weather and animated ambience.
 ...
 for(let p of projectiles){let pp=worldToScreen(p.x,p.y);if(p.kind==='beam'||p.kind==='laserBeam'||(pp.x>-120&&pp.x<W+120&&pp.y>-120&&pp.y<H+120))drawProjectile(p)}for(let p of enemyShots){let q=worldToScreen(p.x,p.y);ctx.fillStyle='#ff5277';ctx.beginPath();ctx.arc(q.x,q.y,p.r,0,TAU);ctx.fill();ctx.strokeStyle='#fff';ctx.stroke()}
 for(let e of enemies){const ep=worldToScreen(e.x,e.y);if(ep.x>-100&&ep.x<W+100&&ep.y>-100&&ep.y<H+100)drawEnemy(e)}if(player.companion){let p=worldToScreen(player.companion.x,player.companion.y);emoji(player.companion.icon,p.x,p.y,27)}drawPlayer();drawParticles();if(G.evolution>0){const a=clamp(G.evolution,0,1);const glow=ctx.createRadialGradient(W/2,H/2,20,W/2,H/2,Math.max(W,H)*.55);glow.addColorStop(0,`rgba(255,244,116,${a*.6})`);glow.addColorStop(.45,`rgba(255,198,43,${a*.18})`);glow.addColorStop(1,'rgba(255,180,20,0)');ctx.fillStyle=glow;ctx.fillRect(0,0,W,H);ctx.globalAlpha=a;ctx.fillStyle='#fff7a0';ctx.font='1000 34px sans-serif';ctx.textAlign='center';ctx.fillText('⭐ WEAPON EVOLVED! ⭐',W/2,H*.27);ctx.globalAlpha=1}ctx.restore()}
...
function loop(now){
 ...
 if(!G.paused){
  G.time+=dt;
  const endless=G.mode==='endless';if(endless&&G.time>=Online.checkpointAt)Online.checkpoint();if(endless&&G.nextBossAt-G.time<=60&&G.nextBossAt-G.time>59.9&&G.lastWarnBoss!==G.nextBossAt){G.lastWarnBoss=G.nextBossAt;tone(120,.5,'sawtooth',.13,280);toast('⚠ Boss Approaching in 60 seconds!')}
  if(endless&&G.time>=G.nextBossAt&&!G.boss){spawnBoss();G.nextBossAt+=600}
  if(endless&&G.time>=G.nextSwarmAt&&!G.swarm)startJunkSwarm();
  if(endless)updateJunkSwarm(dt);
  if(!endless&&G.time>=G.duration&&!G.boss)spawnBoss();
  // Enemy pressure rises smoothly each minute, with a hard population cap.
  spawnClock-=dt;
  if((endless||G.time<G.duration)&&!G.swarm&&spawnClock<=0&&enemies.length<(endless?450:220)){
   const mins=G.time/60;
   // Spawn pressure is gentle for two minutes, spikes through mid game, and
   // then grows slowly with a cap. The cap still overwhelms an idle player.
   const intensity=endless?(mins<2?.82+mins*.18:mins<10?1.18+(mins-2)*.38:4.22+Math.sqrt(mins-10)*.20):(.8+G.stage*.13+G.time/G.duration*2.2);
   const amount=Math.min(endless?5:5,1+Math.floor(intensity/2));
   for(let i=0;i<amount&&enemies.length<(endless?450:220);i++)spawnEnemy(pick(availableEnemies()));
   spawnClock=Math.max(endless?.16:.13,1.05/intensity);
  }
  if(G.quizDone<G.quizAt.length&&G.time>=G.quizAt[G.quizDone]){G.quizDone++;if(endless&&G.quizDone===G.quizAt.length)G.quizAt.push(G.time+240);showQuiz()}
  G.evolution=Math.max(0,(G.evolution||0)-dt);G.bossEntrance=Math.max(0,(G.bossEntrance||0)-dt);
  updatePlayer(dt);updateWeapons(dt);updateUltimates(dt);rebuildSpatialGrid();updateProjectiles(dt);updateEnemies(dt);updateEnemyShots(dt);updateParticles(dt);
...
function updateHud(){
 ...
 if(endless){
  $('#timerText').textContent=formatClock(G.time);$('#stageText').textContent='♾ ENDLESS · SURVIVAL TIME';
...
function updateRecords(){
 ...
 if(G.mode==='endless'){
  ...
  for(const [key,val,label] of [['longestTime',time,'Longest Survival'],['highestScore',score,'Highest Score'],['highestLevel',player.level,'Highest Level']])if(val>r[key]){r[key]=val;G.newRecords.push(label)}
 }else{
